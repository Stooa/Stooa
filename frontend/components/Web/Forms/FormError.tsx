    ...
    {errors.graphQLErrors &&
      errors.graphQLErrors.map((message, key) => (
        <span key={`message-${key}`}>
          {message.debugMessage ? FormErrorParser.parse(message.debugMessage) : FormErrorParser.parse(message.message)}
        </span>
      ))}
    {!errors.graphQLErrors &&
      errors.message
        .split('\n')
        .map((message, key) => <span key={`message-${key}`}>{ FormErrorParser.parse(message) }</span>)}
  </FormErrorStyled>
);
